...
export default{
    ...
    // 求剩余多少天
    getDayAndTime(timeNum){
      // console.log(val1 , val2);
      let num = timeNum/1000
      // console.log(num);
    ...
    // 数组遍历求和
    arrSummation(arr, key){
      let sum = 0
      if(arr && Array.isArray(arr) && arr.length>0){
        arr.forEach((re)=>{
          if(re[key] && !isNaN(Number(re[key]))){
            sum += Number(re[key])
          }
        })        
      }
    ...
    setChannelId(fc){
      //渠道设置
      setTDChannelId(fc)//channelId为字符串形式，setTDChannelId 的调用时机应当在quickInit之前
      //SDK初始化
      quickInit();
